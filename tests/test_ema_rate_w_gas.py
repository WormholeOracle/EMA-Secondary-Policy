...
@pytest.fixture
def ema_policy_reverting(deployer, mock_factory, reverting_rate_calculator, mock_borrowed_token):
...
@pytest.mark.gas_profile
def test_try_catch_safety_mechanism(mock_controller, ema_policy_reverting, reverting_rate_calculator):
    ...
    try:
        ...
        pytest.fail(f"❌ FAILED: rate_write() reverted when it shouldn't have: {str(e)}")
