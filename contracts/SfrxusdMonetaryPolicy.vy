...
prev_ma_sfrxusd_rate: uint256
prev_sfrxusd_rate: uint256
...
def __init__(factory: IFactory, sfrxusd: IFraxVault, borrowed_token: ERC20,
    ...
    r: uint256 = self.raw_sfrxusd_rate()
    self.prev_sfrxusd_rate = r
...
def raw_sfrxusd_apr() -> uint256:
    return self.raw_sfrxusd_rate() * (365 * 86400)


@internal
@view
def ema_sfrxusd_rate() -> uint256:
...
@external
@view
def ma_sfrxusd_rate() -> uint256:
    return self.ema_sfrxusd_rate()
...
def ema_sfrxusd_rate_w() -> uint256:
    ...
    self.prev_sfrxusd_rate = self.raw_sfrxusd_rate()
    self.last_timestamp = block.timestamp
    return r
...
def rate(_for: address = msg.sender) -> uint256:
    return self.calculate_rate(_for, 0, 0, self.ema_sfrxusd_rate())


@external
def rate_write(_for: address = msg.sender) -> uint256:
    return self.calculate_rate(_for, 0, 0, self.ema_sfrxusd_rate_w())
